...
That's it - no need to run anything from the Apps Script editor. The first time you actually use the tool (next section), Google will ask you to grant permissions.
...
You can run it as many times as you like - it's safe to re-run. Brackets that were already fixed are skipped, so running twice never stacks duplicate marks.
...
Running the tool any number of times produces the same result as running it once - re-runs never accumulate duplicate marks. This falls out of the pattern itself rather than needing a separate guard: the pattern only matches when a bracket is preceded by a *letter or space*. Once a mark has been inserted right before a bracket, the character immediately before it is now the mark - neither a letter nor a space - so the pattern no longer matches there on the next run. The fix quietly disables its own trigger.
